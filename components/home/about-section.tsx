        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          >
            ...
            <motion.div
              ...
              className="absolute -bottom-10 -right-10 bg-white dark:bg-slate-950 p-10 rounded-[2.5rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.2)] max-w-[280px] z-20 border border-slate-50"
            >
              <div className="w-16 h-16 bg-primary/10 rounded-2xl flex items-center justify-center mb-6">
                <Award className="text-primary h-8 w-8" />
              </div>
              <h4 className="text-2xl font-bold text-secondary dark:text-white mb-3">NABL Accredited</h4>
              <p className="text-slate-500 dark:text-slate-400 font-medium leading-relaxed">Technical competence in accordance with ISO/IEC 17025.</p>
            </motion.div>
            ...
            <div className="grid gap-10 mb-12">
              {[
                ...
                <div key={item.title} className="flex space-x-6 group">
                  <div className="mt-1 flex-shrink-0 w-12 h-12 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 flex items-center justify-center group-hover:bg-primary group-hover:border-primary transition-all duration-300">
                    <CheckCircle2 className="h-6 w-6 text-primary group-hover:text-white transition-colors" />
                  </div>
                  <div>
                    <h4 className="text-2xl font-bold text-secondary dark:text-white mb-2 group-hover:text-primary transition-colors">{item.title}</h4>
                    <p className="text-lg text-slate-500 dark:text-slate-400 leading-relaxed">{item.content}</p>
                  </div>
                </div>
              ))}
            </div>
